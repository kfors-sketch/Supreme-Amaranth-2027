 ...
 */
export async function getStripe(modeOverride = "") {
  let mode = "test";
  try {
    mode = modeOverride || await getEffectiveOrderChannel();
  } catch (e) {
    console.error(
 ...
 */
export function getStripePublishableKey(mode = "test") {
  const env = process.env || {};
 ...
 * }
 */
export async function getCheckoutSettingsRaw() {
...
export async function saveCheckoutSettings(patch = {}) {
 ...
 */
export async function getCheckoutSettingsAuto(now = new Date()) {
 ...
 */
export async function getEffectiveOrderChannel(now = new Date()) {
